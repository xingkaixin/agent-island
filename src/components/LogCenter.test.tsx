...
function renderLogCenter(nextEntries: TimelineLogEntry[] = entries) {
  ...
  const onLogsCleared = vi.fn<() => void>();
...
describe('LogCenter', () => {
  ...
  it('有筛选条件时，确认文案明确会清空全部日志', async () => {
    ...
    expect(
      within(
        screen.getByText('确认清空日志').closest('.log-danger-panel') as HTMLElement,
      ).getByText('当前已应用筛选条件，但确认后仍会清空全部日志，不仅是当前筛选结果。'),
    ).toBeInTheDocument();
  });
